...
func TestStatsEndpointReturnsCountersWithoutDumpingPeers(t *testing.T) {
	...
	// A test host has no awg0, so counters are unreadable and the handler
	// reports failure rather than inventing zeroes. Either way it must answer
	// quickly and never expose peer detail, which is what is asserted above.
	if resp.Success && resp.Data.Interface != wgParams.ServerWGNIC {
		t.Fatalf("unexpected interface in stats response: %s", body)
	}
}
